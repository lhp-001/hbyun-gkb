...
public interface TblStaffMySqlMapper extends MySqlMapper<TblStaff> {

	@InsertProvider(type = TblStaffMySqlConfig.class,method = "insertStaffReturnStaffId")
	...
	@InsertProvider(type = TblStaffMySqlConfig.class,method = "insertManagerRight")
	void insertManagerRight(@Param("rightList") List<String> rightList,@Param("staffid") BigDecimal staffid);
	
	@Select("SELECT TSF.EMAIL,TSF.ADDRESS,TSF.MIBLEPHONE,TOG.ORGNAME,TJ.JOBNAME,(SELECT ORGNAME FROM tbl_organization WHERE ORGID = (SELECT getParentOrgList(TSF.ORGID))) AS ORGFANAME FROM TBL_STAFF TSF LEFT JOIN TBL_JOB TJ ON TJ.JOBID = TSF.JOBID LEFT JOIN TBL_ORGANIZATION TOG ON TSF.ORGID = TOG.ORGID WHERE TSF.STAFFID = #{staffId}")
	@Results({
	...
	@SelectProvider(type = TblStaffMySqlConfig.class,method = "selectUserInfoByOrgId")
	@Results({
	...
	@Select("SELECT * FROM TBL_STAFF WHERE CREATETIME = ("
			+ " SELECT MIN(CREATETIME) FROM TBL_STAFF WHERE (FIND_IN_SET(ORGID,(SELECT getChildrenDeptList(${orgId}))) OR ORGID = ${orgId})) AND "
			+ " (FIND_IN_SET(ORGID,(SELECT getChildrenDeptList(${orgId}))) OR ORGID = ${orgId} ) ORDER BY CREATETIME ASC LIMIT 0,1")
	TblStaff selectWxappAdmin(@Param("orgId")BigDecimal orgId) throws Exception;

	@SelectProvider(type = TblStaffMySqlConfig.class,method = "selectUserInfoByDeptIdIncludOrg")
	@Results({
		@Result(column="STAFFID",property="staffid"),
		@Result(column="REALNAME",property="realname"),
		@Result(column="ORGNAME",property="orgName"),
